...
export default function MovieScreen() {
  ...
  // console.log("MovieScreen: ", item)
  // console.log("MovieScreen ID: ", item.id)
  ...
  //     setIsFavorite(favoriteMovies.includes(item.id));
  //   }
  //   checkFavorite();
  // }, [item.id]);
    ...
    <ScrollView contentContainerStyle={{ paddingBottom: 20 }} className='flex-1 bg-neutral-900'>
      ...
      {loading ? (
        ...
        <>
          <View>
            <CustomImage
              initialSource={posterImg}
              fallbackImage={fallbackImg}
              contentFit={"cover"}
              style={{ width: Global_width, height: Global_height * 0.6 }}
            />
          ...
          <View style={{ marginTop: -(Global_height * 0.09) }} className='space-y-3'>
                ...
                movieDetails?.genres?.map((genre, index) => {
                  return (
                    <Text 
                      ...
                      onPress={() => navigation.push("Genre", { id: genre.id, name: genre.name, type: "movie" })}
                    >
            ...
            {similarMoviesLoading ? (
              ...
              similarMovies?.length > 0 && <MovieList title='Similar Movies' type='movie' data={similarMovies} hideSeeAll={true} />
            )}
          </View>
        </>
      )}
